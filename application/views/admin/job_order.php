						...
						<div class="row">
							<table class="table table-striped col-md-12">
								<tbody>
									...
									<tr>
										<td><strong>KRITERIA <font color="#FF0000">*</font></strong></td>
										<td>
											<textarea class="form-control" id="kriteria_modal" name="kriteria_modal" placeholder="Kriteria karyawan" rows="3"></textarea>
											<span id='pesan_kriteria_modal'></span>
										</td>
									</tr>
									<tr>
										<td><strong>JOBDESC <font color="#FF0000">*</font></strong></td>
										<td>
											<textarea class="form-control" id="jobdesc_modal" name="jobdesc_modal" placeholder="Deskripsi pekerjaan" rows="3"></textarea>
											<span id='pesan_jobdesc_modal'></span>
										</td>
									</tr>
									<tr>
										<td><strong>BENEFIT <font color="#FF0000">*</font></strong></td>
										<td>
											<textarea class="form-control" id="benefit_modal" name="benefit_modal" placeholder="Benefit yang diterima karyawan" rows="3"></textarea>
											<span id='pesan_benefit_modal'></span>
										</td>
									</tr>
								</tbody>
							</table>
...
<script type='text/javascript'>
	...
	$(document).ready(function() {
		...
		tabel_jo = $('#jo-datatables').DataTable({
			...
			'ajax': {
				'url': '<?= base_url() ?>admin/job_order/list_jabatan_jo',
				data: {
					[csrfName]: csrfHash,
					id_master_jo: id_master_jo,
					// jabatan: jabatan,
					// region: region,
					// rekruter: rekruter,
...
<script>
	function add_job_order() {
		...
		$('#jabatan_input_modal').val("").change();
		$('#kriteria_modal').val("");
		$('#jobdesc_modal').val("");
		$('#benefit_modal').val("");

		$("#jabatan_input_modal").removeAttr('disabled');
		$("#kriteria_modal").removeAttr('disabled');
		$("#jobdesc_modal").removeAttr('disabled');
		$("#benefit_modal").removeAttr('disabled');

		//inisialisasi pesan
		$('#pesan_jabatan_input_modal').html("");
		$('#pesan_kriteria_modal').html("");
		$('#pesan_jobdesc_modal').html("");
		$('#pesan_benefit_modal').html("");
...
<script type="text/javascript">
	function save_JO() {
		//-------ambil isi variabel-------
		var jabatan_id = $("#jabatan_input_modal").val();
		var jabatan_name = $("#jabatan_input_modal option:selected").text();
		var kriteria = $("#kriteria_modal").val();
		var jobdesc = $("#jobdesc_modal").val();
		var benefit = $("#benefit_modal").val();

		jabatan_name = jabatan_name.trim();
		...
		var pesan_jabatan_input_modal = "";
		var pesan_kriteria_modal = "";
		var pesan_jobdesc_modal = "";
		var pesan_benefit_modal = "";
		if (benefit == "") {
			pesan_benefit_modal = "<small style='color:#FF0000;'>Benefit tidak boleh kosong</small>";
			$('#benefit_modal').focus();
		}
		if (jobdesc == "") {
			pesan_jobdesc_modal = "<small style='color:#FF0000;'>Job description tidak boleh kosong</small>";
			$('#jobdesc_modal').focus();
		}
		if (kriteria == "") {
			pesan_kriteria_modal = "<small style='color:#FF0000;'>Kriteria tidak boleh kosong</small>";
			$('#kriteria_modal').focus();
		}
		if (jabatan_id == "") {
			pesan_jabatan_input_modal = "<small style='color:#FF0000;'>Jabatan tidak boleh kosong</small>";
			$('#jabatan_input_modal').focus();
		}
		$('#pesan_jabatan_input_modal').html(pesan_jabatan_input_modal);
		$('#pesan_kriteria_modal').html(pesan_kriteria_modal);
		$('#pesan_jobdesc_modal').html(pesan_jobdesc_modal);
		$('#pesan_benefit_modal').html(pesan_benefit_modal);

		//-------action-------
		if (
			(pesan_jabatan_input_modal != "") || (pesan_kriteria_modal != "") ||
			(pesan_jobdesc_modal != "") || (pesan_benefit_modal != "")
			...
			// alert("Data ADD OK");
			$.ajax({
				url: '<?= base_url() ?>admin/Job_order/add_jabatan_JO/',
				method: 'post',
				data: {
					[csrfName]: csrfHash,
					id_master_jo: id_master_jo,
