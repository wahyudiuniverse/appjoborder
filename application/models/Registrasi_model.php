...
class Registrasi_model extends CI_model
{
	...
	//ambil data kandidat berdasarkan nik nya
	public function cek_nik($nik)
	{
		//$otherdb = $this->load->database('default', TRUE);
		$id_kandidat_terakhir = $this->get_id_kandidat_by_nik($nik);

		$this->db->select('*');
		$this->db->from('kandidat');
		$this->db->where('id', $id_kandidat_terakhir);
	...
	//ambil id kandidat terakhir berdasarkan nik nya
	public function get_id_kandidat_by_nik($nik)
	{
		$this->db->select('max(id) as maxid');
		$this->db->from('kandidat');
		$this->db->where('nik', $nik);
		$this->db->where('status_finish', '1');
	...
	public function getAllCompany()
	{
		...
		$query = $database_cis->get('xin_companies')->result_array();
	...
	public function getAllJabatan()
	{
		...
		$database_cis->select('*');
		$database_cis->from('xin_designations');
		$database_cis->where('level >=', 'C1');
	...
	public function get_project_vacant_json()
	{
		//$otherdb = $this->load->database('default', TRUE);
	...
	public function save_data_diri($datarequest)
	{
		//save data
		$this->db->insert('kandidat', $datarequest);
		$insert_id = $this->db->insert_id();
